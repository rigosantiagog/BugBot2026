...
void giro_actualizarYaw() {
  ...
  // *** CORREGIDO (ver conversacion con Claude, log robot_motor__12_.log) ***
  // Se confirmo con datos reales que "giro horario" (+1.0) producia SIEMPRE yaw
  // NEGATIVO (-120.8°, -121.8° en las 2 repeticiones) y "giro antihorario" (-1.0)
  // SIEMPRE yaw POSITIVO (+134.9°, +135.7°) -- exactamente al reves de lo que dice
  // el comentario de este modulo ("Positivo = giro horario"). Esto rompia
  // corregirOrientacionHaciaNorte(): las 2 veces que corrio en el log, en vez de
  // acercar el yaw a 0 lo alejaba (de -121.96° a 179.91°, y de -70.70° a 179.9°,
  // agotando el watchdog de 10s ambas veces sin alinearse). Se invierte el signo
  // aqui para que "positivo=horario" sea cierto de verdad, y con eso la logica de
  // corregirOrientacionHaciaNorte() (que no se toco) vuelve a funcionar como esta
  // escrita.
