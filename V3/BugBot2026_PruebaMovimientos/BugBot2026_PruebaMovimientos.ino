...
void setup() {
  ...
  Debug.println("[SETUP] Paso 1/3: Inicializando subsistema de motores...");
  motores_inicializar();
  Debug.println("[SETUP] Paso 1/3: Motores OK");
  Debug.println();

  // --- Paso 2: Inicializa el giroscopio ---
  Debug.println("[SETUP] Paso 2/3: Inicializando giroscopio MPU6500...");
  giro_inicializar();
  Debug.println("[SETUP] Paso 2/3: Giroscopio OK");
  Debug.println();
  ...
  Debug.println("[SETUP] Paso 3/3: Calibracion del giroscopio...");
  ...
  Debug.println("[SETUP] Paso 3/3: Calibracion OK");
  Debug.println();

  Debug.println("[SETUP] *** INICIALIZACION COMPLETA ***");
  Debug.println("[SETUP] El robot comenzara a moverse en 2 segundos...");
  delay(2000);
}
...
void loop() {
  ...
  Debug.println();
  Debug.print("[LOOP] Pausa final de ");
  Debug.print(PAUSA_FINAL_MS);
  Debug.println(" ms antes de verificar orientacion...");
  ...
  Debug.println();
  Debug.println("[LOOP] === VERIFICACION DE ORIENTACION ===");
  Debug.print("[LOOP] Yaw al finalizar: ");
  Debug.print(yawFinal, 2);
  Debug.println(" grados");
  Debug.print("[LOOP] Tolerancia: +/- ");
  Debug.print(TOLERANCIA_ORIENTACION_GRADOS, 1);
  Debug.println(" grados");

  // --- Paso D: Corrige o reinicia según la orientación ---
  if (giro_estaAlineadoConNorte()) {
    Debug.println("[LOOP] RESULTADO: Robot ALINEADO. Reiniciando secuencia...");
  } else {
    Debug.print("[LOOP] RESULTADO: Robot DESVIADO (");
    Debug.print(yawFinal, 2);
    Debug.println(" grados). Corrigiendo...");
    corregirOrientacionHaciaNorte();
    Debug.println("[LOOP] Correccion completada. Reiniciando secuencia...");
  }
