...
void ejecutarMovimiento(const char* nombre, float adelante, float lateral, float giro) {
  Debug.println("--------------------------------------------------");
  Debug.print("[MOV] INICIANDO: ");
  Debug.println(nombre);                         // Nombre descriptivo del movimiento
  Debug.print("[MOV] Params -> adelante=");
  Debug.print(adelante, 1);                      // Componente adelante/atrás (-1 a +1)
  Debug.print("  lateral=");
  Debug.print(lateral, 1);                       // Componente lateral (-1 a +1)
  Debug.print("  giro=");
  Debug.print(giro, 1);                          // Componente de rotación (-1 a +1)
  Debug.print("  PWM objetivo=");
  Debug.print(VELOCIDAD_PRUEBA);                 // Velocidad PWM máxima del movimiento
  Debug.print("  rampa=");
  Debug.print(RAMPA_DURACION_MS);
  Debug.println("ms");
  ...
  Debug.println("[MOV] Fase 1: Rampa de aceleracion...");
  mover_mecanum_con_rampa(adelante, lateral, giro, VELOCIDAD_PRUEBA, giro_actualizarYaw);
  Debug.println("[MOV] Fase 2: Velocidad de crucero...");
  ...
  while (millis() - tInicio < DURACION_MOVIMIENTO_MS) {
    ...
    if (millis() - tWatchdog > WATCHDOG_MOV_MS) {
      Debug.print("[MOV] *** WATCHDOG en '");
      Debug.print(nombre);
      Debug.print("' t=");
      Debug.print(millis() - tWatchdog);
      Debug.println("ms — salida forzada ***");
    ...
    if (iteraciones % 200 == 0) {
      ...
      Debug.print(nombre);
      Debug.print("'  t=");
      Debug.print(millis() - tInicio);           // Tiempo total transcurrido (ms)
      Debug.print("ms  yaw=");
      Debug.print(giro_obtenerYaw(), 2);          // Orientación actual (grados)
      Debug.print((char)176);                     // Símbolo de grado '°'
      Debug.print("  I2C=");
      Debug.println(giro_busOK() ? "OK" : "FALLO"); // Estado del bus I2C
    }
  }
  ...
  Debug.print("[MOV] TERMINADO: '");
  Debug.print(nombre);
  Debug.print("'  yaw=");
  Debug.print(giro_obtenerYaw(), 2);
  Debug.print((char)176);
  Debug.print("  duracion=");
  Debug.print(millis() - tInicio);
  Debug.println("ms");

  // Pausa entre movimientos: da tiempo al bus I2C de estabilizarse
  Debug.print("[MOV] Pausa: ");
  Debug.print(PAUSA_ENTRE_MOVS_MS);
  Debug.println("ms");
...
void ejecutarSecuenciaCompletaDeMovimientos() {
  ...
  Debug.println("==================================================");
  Debug.println("[SEQ] INICIANDO SECUENCIA COMPLETA DE MOVIMIENTOS");
  Debug.println("[SEQ] Total de movimientos: 12");
  Debug.println("==================================================");

  // --- GRUPO 1: Movimientos lineales puros ---
  // Solo componente adelante/atrás, sin lateral ni giro
  Debug.println("[SEQ] === GRUPO 1: Movimientos lineales ===");
  ...
  Debug.println("[SEQ] === GRUPO 2: Movimientos laterales (exclusivo mecanum) ===");
  ...
  Debug.println("[SEQ] === GRUPO 3: Movimientos diagonales ===");
  ...
  Debug.println("[SEQ] === GRUPO 4: Rotaciones in situ ===");
  ...
  Debug.println("[SEQ] === GRUPO 5: Movimientos curvos ===");
  ...
  Debug.println("==================================================");
  Debug.println("[SEQ] SECUENCIA COMPLETA FINALIZADA (12/12 movimientos)");
  Debug.print("[SEQ] Yaw acumulado al finalizar: ");
  Debug.print(giro_obtenerYaw(), 2);
  Debug.println(" grados");
  Debug.println("==================================================");
}
...
void corregirOrientacionHaciaNorte() {
  Debug.println("--------------------------------------------------");
  Debug.println("[COR] Robot FUERA de orientacion inicial. Iniciando correccion...");
  Debug.print("[COR] Yaw actual: ");
  Debug.print(giro_obtenerYaw(), 2);
  Debug.print((char)176);                       // Símbolo de grados
  Debug.print("  Tolerancia: +/-");
  Debug.print(TOLERANCIA_ORIENTACION_GRADOS, 1);
  Debug.println((char)176);
  ...
  while (!giro_estaAlineadoConNorte()) {
    ...
    if (millis() - tInicio > WATCHDOG_CORRECCION_MS) {
      Debug.print("[COR] *** WATCHDOG: correccion abortada tras ");
      Debug.print(WATCHDOG_CORRECCION_MS / 1000);
      Debug.println(" segundos — posible fallo del giroscopio ***");
      Debug.print("[COR] Yaw al abortar: ");
      Debug.print(giro_obtenerYaw(), 2);
      Debug.println((char)176);
    ...
    if (millis() - tUltimoPrint > 300) {
      Debug.print("[COR] Yaw: ");
      Debug.print(yawActual, 2);
      Debug.print((char)176);
      Debug.print("  Girando: ");
      Debug.print(sentido > 0 ? "HORARIO" : "ANTIHORARIO");
      Debug.print("  Velocidad: ");
      Debug.print(VELOCIDAD_CORRECCION_GIRO);
      Debug.print("  Tiempo: ");
      Debug.print(millis() - tInicio);
      Debug.println("ms");
  ...
  Debug.print("[COR] Correccion FINALIZADA. Yaw final: ");
  Debug.print(giro_obtenerYaw(), 2);
  Debug.print((char)176);
  Debug.print("  Tiempo total: ");
  Debug.print(millis() - tInicio);
  Debug.println("ms");
  Debug.println("--------------------------------------------------");
}
