 ...
 *
 *  CAMBIO v6 (revertido: brownout detector REACTIVADO):
 *   - El log de pruebas mostró bloques de "Motivo del ultimo reinicio:
 *     Encendido normal (power-on)" repetidos ENTRE bytes de basura binaria.
 *     Eso confirma que, aun con el detector de brownout desactivado por
 *     software, la caida de voltaje al arrancar los motores es tan fuerte
 *     que dispara el circuito ANALOGICO de power-on-reset del chip (que no
 *     se puede desactivar). El registro que se escribia aqui solo apagaba
 *     el detector DIGITAL — y mientras tanto, con voltaje insuficiente, la
 *     ESP32 seguia "corriendo" en un estado invalido: de ahi la basura en
 *     el serial, justo antes del reinicio duro que igual terminaba pasando.
 *   - Al reactivar el detector digital, la ESP32 se reinicia LIMPIO en
 *     cuanto el voltaje cae del umbral (~2.4V), en vez de seguir ejecutando
 *     con voltaje insuficiente y corromper UART/memoria antes de reiniciarse
 *     de todos modos. Esto NO evita el reinicio por si solo (eso requiere
 *     arreglar la caida de voltaje en hardware — ver config_robot.h e
 *     igualmente subir capacitores de bulk en cada TB6612FNG), pero hace
 *     que el reinicio sea inmediato, diagnosticable (imprimirRazonReset()
 *     mostrara "BROWNOUT" en vez de "power-on") y no deje al robot
 *     ejecutando codigo corrupto ni un instante.
 * ============================================================================
 */

// Incluye el driver del detector de brownout del ESP32 (parte del IDF)
// Se deja incluido aunque ya NO se desactive el detector: esp_system.h /
// las macros de este header pueden usarse a futuro para leer su estado.
...
// ============================================================================
// imprimirRazonReset()
// Consulta al propio chip la causa de su ultimo reinicio y la imprime por el
// monitor serial. Con el brownout detector REACTIVADO, un reinicio causado
// por caida de voltaje ahora se reporta como "BROWNOUT" (antes, con el
// detector digital desactivado, este mismo evento se veia como un reinicio
// "power-on" generico disparado por el circuito analogico de respaldo).
// ============================================================================
void imprimirRazonReset() {
  ...
  switch (razon) {
    ...
    case ESP_RST_BROWNOUT: Debug.println("BROWNOUT: caida de voltaje detectada por hardware"); break;     // Confirma que fue voltaje: revisar alimentacion/capacitores
...
void setup() {
  ...
  // --- CAMBIO CRÍTICO v6: el detector de brownout YA NO se desactiva ---
  // Antes aqui se escribia WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0) para
  // apagar el detector de brownout por software, con la idea de tolerar
  // caidas momentaneas de voltaje sin reiniciarse. En la practica, el log
  // de pruebas mostro que la caida de voltaje al arrancar motores es tan
  // fuerte que el chip terminaba reiniciandose de todos modos (via el
  // circuito analogico de respaldo que NO se puede apagar), pero solo
  // DESPUES de haber corrido un rato con voltaje insuficiente y corromper
  // el UART (la "basura" que aparecia en el monitor serial).
  //
  // Al dejar el detector de brownout ACTIVO (comportamiento por defecto del
  // ESP32, no se requiere ninguna linea de codigo), el chip se reinicia de
  // forma LIMPIA e INMEDIATA en cuanto el voltaje cae del umbral (~2.4V),
  // en vez de seguir ejecutando codigo con datos corruptos. El reinicio en
  // si NO desaparece con este cambio -- eso depende de resolver la caida de
  // voltaje en hardware (ver SECCION 7 de config_robot.h: RAMPA_DURACION_MS
  // y VELOCIDAD_PRUEBA, y agregar capacitores de bulk en cada TB6612FNG).
  Debug.println("[SISTEMA] Brownout detector ACTIVO (comportamiento por defecto del ESP32)");
  ...
  Debug.println("#      v5: Brownout detector activo + rampa mas larga      #");
  Debug.println("############################################################");
  Debug.println();
