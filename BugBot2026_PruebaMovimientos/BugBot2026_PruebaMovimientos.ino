...
#include "movimientos.h"    // Maniobras y corrección de orientación
...
void imprimirRazonReset() {
  ...
  Serial.print("Motivo del ultimo reinicio: ");  // Etiqueta para ubicar el dato en el monitor serial
  switch (razon) {
    case ESP_RST_POWERON:  Serial.println("Encendido normal (power-on)"); break;                          // Arranque limpio, no hubo falla previa
    case ESP_RST_BROWNOUT: Serial.println("BROWNOUT: caida de voltaje detectada por hardware"); break;     // Confirma que SI fue voltaje (aunque el detector por software este deshabilitado)
    case ESP_RST_TASK_WDT: Serial.println("WATCHDOG DE TAREA: el loop() se bloqueo demasiado tiempo"); break; // Apunta a codigo colgado, ej. I2C sin timeout efectivo
    case ESP_RST_INT_WDT:  Serial.println("WATCHDOG DE INTERRUPCION: una interrupcion tardo demasiado"); break; // Puede indicar una ISR bloqueada
    case ESP_RST_PANIC:    Serial.println("PANICO: crash de software (ej. acceso a memoria invalido)"); break; // Bug de codigo, no relacionado a voltaje
    case ESP_RST_SW:       Serial.println("Reinicio por software (ESP.restart())"); break;                 // Reinicio intencional del propio programa
    default:                Serial.printf("Otro motivo, codigo: %d\n", razon); break;                       // Cualquier caso no listado arriba
  }
}
...
void setup() {
  ...
  imprimirRazonReset();
  Serial.println("[SISTEMA] Brownout detector DESACTIVADO por software");

  Serial.println();
  Serial.println("############################################################");
  Serial.println("#      BugBot 2026 - Prueba de Movimientos Mecanum         #");
  Serial.println("#      Giroscopio MPU6500 - ESP32 Arduino core 3.x         #");
  Serial.println("#      v4: Freno activo real + diagnostico de reinicio     #");
  Serial.println("############################################################");
  Serial.println();

  // --- Paso 1: Inicializa los motores ---
  Serial.println("[SETUP] Paso 1/3: Inicializando subsistema de motores...");
  motores_inicializar();
  Serial.println("[SETUP] Paso 1/3: Motores OK");
  Serial.println();

  // --- Paso 2: Inicializa el giroscopio ---
  Serial.println("[SETUP] Paso 2/3: Inicializando giroscopio MPU6500...");
  giro_inicializar();
  Serial.println("[SETUP] Paso 2/3: Giroscopio OK");
  Serial.println();
  ...
  Serial.println("[SETUP] Paso 3/3: Calibracion del giroscopio...");
  Serial.println("[SETUP] *** COLOCA EL ROBOT EN SU POSICION INICIAL Y NO LO MUEVAS ***");
  Serial.println("[SETUP] Esperando 5 segundos para estabilizacion de la fuente...");
  delay(5000);  // 5 segundos (antes eran 3) para mayor estabilidad

  giro_calibrarYFijarNorte();
  Serial.println("[SETUP] Paso 3/3: Calibracion OK");
  Serial.println();

  Serial.println("[SETUP] *** INICIALIZACION COMPLETA ***");
  Serial.println("[SETUP] El robot comenzara a moverse en 2 segundos...");
  delay(2000);
}
...
void reiniciarBusI2C() {
  Serial.println("[I2C] Demasiados fallos consecutivos — reiniciando bus I2C...");
  ...
  Serial.println("[I2C] Bus I2C reiniciado correctamente.");
}

// ============================================================================
// loop()
// Ciclo principal que se repite indefinidamente.
// ============================================================================
void loop() {
  // --- Paso A: Ejecuta la secuencia completa de movimientos ---
  ejecutarSecuenciaCompletaDeMovimientos();

  // Verifica el estado del bus I2C después de la secuencia
  // (los motores pueden haber generado suficiente ruido para degradar el bus)
  if (!giro_busOK()) {
    _fallosI2CConsecutivos++;
    if (_fallosI2CConsecutivos >= MAX_FALLOS_I2C_ANTES_DE_RESET) {
      reiniciarBusI2C();  // Recupera el bus I2C si está degradado
    }
  } else {
    _fallosI2CConsecutivos = 0;  // Resetea el contador si la última lectura fue OK
  }

  // --- Paso B: Pausa final de 2 segundos ---
  Serial.println();
  Serial.print("[LOOP] Pausa final de ");
  Serial.print(PAUSA_FINAL_MS);
  Serial.println(" ms antes de verificar orientacion...");
  delay(PAUSA_FINAL_MS);

  // --- Paso C: Verifica la orientación ---
  giro_actualizarYaw();
  float yawFinal = giro_obtenerYaw();

  Serial.println();
  Serial.println("[LOOP] === VERIFICACION DE ORIENTACION ===");
  Serial.print("[LOOP] Yaw al finalizar: ");
  Serial.print(yawFinal, 2);
  Serial.println(" grados");
  Serial.print("[LOOP] Tolerancia: +/- ");
  Serial.print(TOLERANCIA_ORIENTACION_GRADOS, 1);
  Serial.println(" grados");

  // --- Paso D: Corrige o reinicia según la orientación ---
  if (giro_estaAlineadoConNorte()) {
    Serial.println("[LOOP] RESULTADO: Robot ALINEADO. Reiniciando secuencia...");
  } else {
    Serial.print("[LOOP] RESULTADO: Robot DESVIADO (");
    Serial.print(yawFinal, 2);
    Serial.println(" grados). Corrigiendo...");
    corregirOrientacionHaciaNorte();
    Serial.println("[LOOP] Correccion completada. Reiniciando secuencia...");
  }
