...
static void _reiniciarBusI2C() {
  Serial.println("[I2C] *** Reiniciando bus I2C por fallos consecutivos ***");
  ...
  Serial.println("[I2C] Bus I2C reiniciado correctamente. Continuando...");
}
...
static bool _escribirRegistro(uint8_t reg, uint8_t valor) {
  ...
  if (codigoError != 0) {
    ...
    Serial.print("[I2C] ERROR al escribir registro 0x");
    Serial.print(reg, HEX);                    // Imprime número de registro en hexadecimal
    Serial.print(" -> codigo: ");
    Serial.println(codigoError);               // Imprime el código de error Wire
...
void giro_inicializar() {
  Serial.println("[GIROSCOPIO] Iniciando modulo de giroscopio...");
  ...
  Serial.println("[GIROSCOPIO] DAC desactivado en GPIO25 (DAC1) y GPIO26 (DAC2)");
  Serial.println("[GIROSCOPIO] GPIO25 y GPIO26 ahora disponibles para PWM/GPIO");
  ...
  Serial.print("[GIROSCOPIO] I2C iniciado -> SDA=GPIO");
  Serial.print(PIN_MPU_SDA);                    // Imprime el número de pin SDA
  Serial.print(", SCL=GPIO");
  Serial.print(PIN_MPU_SCL);                    // Imprime el número de pin SCL
  Serial.println(", frecuencia=100 kHz (modo estandar, alta inmunidad a ruido)");
  ...
  Serial.println("[GIROSCOPIO] Despertando MPU6500 del modo sleep...");
  if (_escribirRegistro(MPU_REG_PWR_MGMT_1, 0x00)) {  // 0x00 = wake up + internal clock
    Serial.println("[GIROSCOPIO] MPU6500 despertado correctamente");
  } else {
    Serial.println("[GIROSCOPIO] *** ADVERTENCIA: No se pudo despertar el MPU6500 ***");
    Serial.println("[GIROSCOPIO] Verifica la conexion VCC, GND, SDA, SCL del sensor");
  }
  ...
  Serial.println("[GIROSCOPIO] Configurando rango del giroscopio...");
  if (_escribirRegistro(MPU_REG_GYRO_CONFIG, 0x00)) {  // 0x00 = rango ±250°/s
    Serial.println("[GIROSCOPIO] Rango configurado: +/- 250 grados/segundo (maxima sensibilidad)");
  }
  delay(50);   // Espera 50ms para que la configuración sea aplicada por el sensor

  // Resumen del estado del módulo
  Serial.println("[GIROSCOPIO] *** MPU6500 inicializado y listo ***");
  Serial.print("[GIROSCOPIO] Timeout I2C configurado: ");
  Serial.print(I2C_TIMEOUT_MS);
  Serial.println(" ms");
  Serial.print("[GIROSCOPIO] Reinicio automatico de bus tras ");
  Serial.print(I2C_FALLOS_PARA_REINICIO);
...
void giro_calibrarYFijarNorte() {
  Serial.println("[GIROSCOPIO] *** CALIBRACION INICIADA ***");
  Serial.println("[GIROSCOPIO] *** NO MOVER EL ROBOT DURANTE LA CALIBRACION ***");
  ...
  for (int i = 0; i < MUESTRAS_CALIBRACION_GIRO; i++) {
    ...
    if (i % 100 == 0) {
      Serial.print("[GIROSCOPIO] Calibrando: ");
      Serial.print(i);                           // Muestra el progreso actual
      Serial.print("/");
      Serial.print(MUESTRAS_CALIBRACION_GIRO);   // Muestra el total de muestras
      Serial.print("  validas: ");
      Serial.print(muestrasOK);                  // Muestra cuántas fueron exitosas
      Serial.print("  offset parcial: ");
      Serial.print(muestrasOK > 0 ? (float)(sumaLecturas / muestrasOK) : 0.0f, 4);
      Serial.println(" grados/s");               // Muestra el offset parcial hasta ahora
    }
  }
  ...
  if (muestrasOK > 0) {
    ...
    Serial.print("[GIROSCOPIO] Offset de cero calculado: ");
    Serial.print(_offsetZ, 5);                   // Imprime con 5 decimales para máxima info
    Serial.println(" grados/segundo");
  } else {
    // Ninguna lectura fue exitosa durante toda la calibración
    _offsetZ = 0.0f;                             // Sin offset calculado
    Serial.println("[GIROSCOPIO] *** ERROR CRITICO: 0 muestras validas en calibracion ***");
    Serial.println("[GIROSCOPIO] Posibles causas:");
    Serial.println("[GIROSCOPIO]   - Sensor MPU6500 no conectado o dañado");
    Serial.println("[GIROSCOPIO]   - Dirección I2C incorrecta (revisar pin AD0)");
    Serial.println("[GIROSCOPIO]   - Problema en bus SDA/SCL (cortocircuito, cables sueltos)");
  }
  ...
  Serial.println("[GIROSCOPIO] *** CALIBRACION COMPLETA ***");
  Serial.print("[GIROSCOPIO] Muestras totales: ");
  Serial.print(MUESTRAS_CALIBRACION_GIRO);
  Serial.print("  exitosas: ");
  Serial.print(muestrasOK);
  Serial.print("  (");
  Serial.print((muestrasOK * 100) / MUESTRAS_CALIBRACION_GIRO); // Porcentaje de éxito
  Serial.println("%)");
  Serial.println("[GIROSCOPIO] Orientacion actual fijada como NORTE INICIAL = 0 grados");
}
...
void giro_actualizarYaw() {
  ...
  if (dt > 0.5f) {
    ...
    Serial.print("[GIROSCOPIO] dt anomalo ignorado: ");
    Serial.print(dt * 1000.0f, 1);          // Imprime dt en milisegundos
    Serial.println(" ms (reinicio de Wire o delay largo previo)");
...
void giro_reiniciarBusManual() {
  Serial.println("[GIROSCOPIO] Reinicio manual del bus I2C solicitado desde codigo externo");
